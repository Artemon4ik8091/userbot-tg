...
9. [Встроенный Telegram Бот ядра и Inline API](#9-встроенный-telegram-бот-ядра-и-inline-api)
...
* 📂 `system_modules/` — Системные модули (ядро команд, хелп, установщик). **Фича ядра:** любой файл, помещенный в эту папку, автоматически получает статус «системного». Такие модули защищены от случайного удаления через `.uninstall`, а в команде `.help` помечаются иконкой 🔒.
