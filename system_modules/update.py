...
set_module_meta(
    name="Обновление",
    desc="Системный модуль обновления юзербота из репозитория Gitea с инлайн-кнопками и фоновым чекером.",
    system=True
)
...
# 🔗 Источник репозитория (только Gitea)
REPO_SOURCES = [
    {
        "name": "Gitea",
        "url": GITEA_REPO_URL
...
async def ensure_git_setup():
    """
    Проверяет готовность git и гарантирует привязку origin к репозиторию Gitea.
...
async def git_fetch_from_gitea(branch, timeout=30):
    """
    Выполняет git fetch только из репозитория Gitea.
    Гарантирует, что origin указывает на репозиторий Gitea.
    Возвращает (success: bool, source_name: str, error_message: str).
    """
    await run_git_cmd("remote", "set-url", "origin", OFFICIAL_REPO_URL)
    logger.debug(f"Попытка git fetch из Gitea ({OFFICIAL_REPO_URL})...")
    code, _, fetch_err = await run_git_cmd("fetch", "origin", branch, timeout=timeout)
    if code == 0:
        logger.info("git fetch успешно выполнен из Gitea")
        return True, "Gitea", ""
    else:
        logger.warning(f"Ошибка при git fetch из Gitea: {fetch_err}")
        return False, "Gitea", f"Gitea: {fetch_err}"


# Для обратной совместимости вызовов
git_fetch_with_fallback = git_fetch_from_gitea
...
async def check_updates_state():
    ...
    logger.debug(f"Текущая ветка: {branch}. Выполняю git fetch из Gitea...")
    fetch_ok, source_name, fetch_err = await git_fetch_from_gitea(branch, timeout=30)
    if not fetch_ok:
        logger.error(f"Ошибка при git fetch: {fetch_err}")
        return {"ok": False, "error": f"Не удалось связаться с репозиторием Gitea:\n`{fetch_err}`"}
...
def build_update_ui(state):
    ...
    if behind_count > 0:
        ...
        text = (
            f"ℹ️ **Локальная версия опережает репозиторий**{src_badge}\n\n"
            f"🌿 **Ветка:** `{branch}`\n"
            f"🔢 **Локальных коммитов:** `{ahead_count}`\n"
            f"📌 **Текущий коммит:** {cur_hash_str} — {cur_msg_str}\n"
            f"👤 **Автор:** `{cur_author_str}` ({cur_date_str})\n\n"
            f"💡 Для синхронизации с репозиторием можно использовать `.update force`."
        )
        buttons = [
            [
        ...
        text = (
            f"✅ **Юзербот обновлен до последней версии!**{src_badge}\n\n"
            f"🌿 **Ветка:** `{branch}`\n"
            f"📌 **Текущий коммит:** {cur_hash_str}\n"
            f"💬 `{cur_msg_str}`\n"
            f"👤 **Автор:** `{cur_author_str}` ({cur_date_str})\n\n"
            f"🔗 [Gitea]({GITEA_REPO_URL})"
        )
        buttons = [
            [Button.inline("🔄 Попробовать снова", b"upd_recheck")]
        ]
...
async def run_update_sequence(client, chat_id, message_id, branch, force=False, event=None):
    ...
    try:
        await update_status("⏳ **Проверяю и скачиваю обновление...**")

        # 1. Fetch из Gitea
        logger.debug(f"Выполняю git fetch для ветки {branch}...")
        fetch_ok, source_name, fetch_err = await git_fetch_from_gitea(branch, timeout=45)
        if not fetch_ok:
            logger.error(f"git fetch завершился с ошибкой: {fetch_err}")
            return await update_status(f"❌ Ошибка подключения к репозиторию Gitea (`git fetch`):\n`{fetch_err}`")

        logger.info(f"Обновление скачивается из источника: Gitea")
            ...
            if code != 0:
                ...
                if "conflict" in (pull_err + pull_out).lower() or "local changes" in (pull_err + pull_out).lower():
                    conflict_hint = (
                        ...
                        "Используйте `.update force`, чтобы перезаписать локальные изменения версией из репозитория."
                    )
        ...
        restart_text = (
            ...
            f"🌐 **Источник:** `Gitea`\n"
            f"👤 **Автор:** `{new_commit['author'] if new_commit else 'UBTG Team'}`"
        )
...
async def cb_upd_recheck(event, data):
    ...
    try:
        await event.edit("🔄 `Проверяю наличие обновлений...`", buttons=None)
    except errors.MessageNotModifiedError:
        pass
...
async def auto_update_checker(client):
    """
    Фоновый процесс: каждые 15 минут проверяет наличие обновлений в репозитории Gitea.
    ...
    while True:
        try:
            ...
            if ready:
                branch = await get_current_branch()
                fetch_ok, source_name, _ = await git_fetch_from_gitea(branch, timeout=30)
                if fetch_ok:
                    ...
                    logger.debug(f"Фоновый чек [{source_name}]: behind={behind_count}, remote={remote_hash[:7] if remote_hash else 'None'}, snoozed={snoozed_hash[:7] if snoozed_hash else 'None'}")
                    ...
                    if (
                        ...
                        logger.info(f"Обнаружено {behind_count} новых коммитов в Gitea! Отправка уведомления владельцу...")
...
async def update_cmd(client, event, args):
    """
    Команда управления обновлениями юзербота из Gitea.
    Использование:
    ...
    if subcmd in ("help", "h", "?"):
        ...
        help_text = (
            ...
            f"🔗 **Репозиторий:** [Gitea]({GITEA_REPO_URL})"
        )
    ...
    await event.edit("🔄 `Проверяю наличие обновлений...`")
...
@register_cmd("checkupdate", desc="Быстрая проверка наличия обновлений из Gitea")
